"""
Everything bug reports, pretty much.
"""
...
class BugReports(commands.Cog):
    ...
    async def create(self, ctx, patch: str = None, link: str = None):
        if patch is None or link is None:
            await ctx.send(
                "The correct format is `.create <version> <patch notes link or Discord>`"
            )
            return
        ...
        async with aiohttp.ClientSession() as session:
            ...
            security_token = await forums.get_security_token(session)
            ...
            thread_mechanics_general_search = (
                f'{patch_search}-Bugs-Mechanics" id="thread_title_'
            )
            ...
            await forums.new_thread(
                session,
                config.HON_FORUM_RCT_BUGS_SUBFORUM_ID,
                thread_mechanics_general,
                message_mechanics_general,
                security_token,
            )
            await forums.new_thread(
                session,
            ...
            await ctx.send(
                f"Threads have been created and can be viewed here: {subforum_url}"
            )
            ...
            async with session.get(subforum_url) as resp:
                ...
                thread_mechanics_general_id = content.split(
                    thread_mechanics_general_search
                )[1][:6]
    ...
    async def report(self, ctx):
        ...
        if report_author in self.OPEN_REPORTS:
            already_pending_content = (
                "You already have a bug report pending, please complete that one first."
            )
            already_pending_message = await report_author.send(
                "{.mention} ".format(report_author) + already_pending_content
            )
        ...
        bc_embed = discord.Embed(
            title="Bug Report Categories", type="rich", color=0xFF6600
        )
        bc_embed.set_author(
            name=report_author_verified_name, icon_url=report_author.avatar_url
        )
        bc_embed.set_footer(
            text="Please add one of the reactions described above to continue."
        )
        ...
        reaction, _ = await self.bot.wait_for(
            "reaction_add",
            check=lambda reaction, user: user == report_author
            and reaction.emoji in reaction_symbols
            and reaction.message.id == bc_message.id,
        )
        ...
        if reaction.emoji == "❌":
            ...
            cancel_report_message = await report_author.send(
                "{.mention} ".format(report_author) + cancel_report_content
            )
            ...
            proceed_prompt_content = "You chose to report a **{}** bug. Do you wish to proceed?".format(
                bug_category
            )
            proceed_prompt_message = await report_author.send(
                "{.mention} ".format(report_author) + proceed_prompt_content
            )
        ...
        reaction, _ = await self.bot.wait_for(
            ...
            and reaction.message.id == proceed_prompt_message.id,
        )
        await proceed_prompt_message.edit(content=proceed_prompt_content)

        if reaction.emoji == "❌":
            proceed_prompt_no_content = (
                "Process cancelled. Use `.report` to start over."
            )
            proceed_prompt_no_message = await report_author.send(
                "{.mention} ".format(report_author) + proceed_prompt_no_content
            )
            self.OPEN_REPORTS.remove(report_author)
            await asyncio.sleep(8)
        ...
        def message_check(message):
            return (
                message.author == report_author
                and message.guild is None
                and not message.content.startswith(("!", "."))
            )  # Commands prefix tuple

        ask_for_version_content = "Please enter the **version** of the RCT client. Format: `<0.xx.xxx>` e.g. `0.27.231.1`"
        ask_for_version_message = await report_author.send(
            "{.mention} ".format(report_author) + ask_for_version_content
        )
        ...
        ask_for_build_date_message = await report_author.send(
            "{.mention} ".format(report_author) + ask_for_build_date_content
        )
        ...
        ask_for_screenshots_message = await report_author.send(
            "{.mention} ".format(report_author) + ask_for_screenshots_content
        )
        number_of_screenshots_message = await self.bot.wait_for(
        ...
        try:
            ...
            if number_of_screenshots > config.HON_FORUM_SCREENSHOT_LIMIT:
                ...
                too_many_screenshots_content = "That might be too many screenshots. Your entry has been changed to `{}`. If the screenshots excluded were necessary, please add them to the description.".format(
                    number_of_screenshots
                )
                too_many_screenshots_message = await report_author.send(
                    "{.mention} ".format(report_author) + too_many_screenshots_content
                )
                await asyncio.sleep(8)
                await too_many_screenshots_message.edit(
                    content=too_many_screenshots_content
                )

            for i in range(1, number_of_screenshots + 1, 1):
                ask_for_screenshot_link_content = "Please enter the **screenshot link** number {} of the bug. e.g. `https://i.imgur.com/D0Yn9JC.png`".format(
                    i
                )
                ask_for_screenshot_link_message = await report_author.send(
                    "{.mention} ".format(report_author)
                    + ask_for_screenshot_link_content
                )
                screenshot_link_message = await self.bot.wait_for(
                    "message", check=message_check
                )
                screenshots_list.append(screenshot_link_message.content)
                await ask_for_screenshot_link_message.edit(
                    content=ask_for_screenshot_link_content
                )
            ...
            screenshots_not_integer_message = await report_author.send(
                "{.mention} ".format(report_author) + screenshots_not_integer_content
            )
            await asyncio.sleep(8)
            await screenshots_not_integer_message.edit(
                content=screenshots_not_integer_content
            )
        ...
        ask_for_description_message = await report_author.send(
            "{.mention} ".format(report_author) + ask_for_description_content
        )
        bug_description_message = await self.bot.wait_for(
            "message", check=message_check
        )
        ...
        if len(screenshots_list) < 1:
            ...
            if len(screenshots_list_message_discord) > 200:
                screenshots_list_message_discord = (
                    screenshots_list_message_discord[:200] + "...\n\n"
                )
        ...
        final_report_prompt_content = "Is this your final report?\n```Version: {0}\nBuild date: {1}\n\n{2}{3}```".format(
            rc_version_message.content,
            build_date_message.content,
            screenshots_list_message_discord,
            bug_description_discord,
        )
        final_report_prompt_message = await report_author.send(
            "{.mention} ".format(report_author) + final_report_prompt_content
        )
        ...
        reaction, _ = await self.bot.wait_for(
            ...
            check=lambda reaction, user: user == report_author
            and reaction.emoji in ["✅", "❌"]
            and reaction.message.id == final_report_prompt_message.id,
        )
        ...
        if reaction.emoji == "❌":
            ...
            final_report_discarded_message = await report_author.send(
                "{.mention} ".format(report_author) + final_report_discarded_content
            )
            self.OPEN_REPORTS.remove(report_author)
            await asyncio.sleep(8)
            await final_report_discarded_message.edit(
                content=final_report_discarded_content
            )
            return

        async with aiohttp.ClientSession() as session:
            await forums.login(session)
            message = "[COLOR=#0099FF]Version:[/COLOR] {0}\n[COLOR=#0099FF]Build date:[/COLOR] {1}\n\n{2}{3}\n\n\nReported by: [COLOR=#00cc99]{4}[/COLOR] ({5.name}#{5.discriminator})".format(
                rc_version_message.content,
                build_date_message.content,
                screenshots_list_message,
                bug_description_message.content,
                report_author_verified_name,
                report_author,
            )
            await forums.new_reply(session, threadid, message)
        ...
        is_this_valid = await bug_reports_channel.send(
            "{0}, your report has been posted in the **{1}** thread and can be viewed here: https://forums.heroesofnewerth.com/showthread.php?{2}&goto=newpost\n```Version: {3}\nBuild date: {4}\n\n{5}{6}\n\n\nReported by: {7} ({8.name}#{8.discriminator})```<@&248187345776410625> Awaiting decision.".format(
                report_author.mention,
                bug_category,
                threadid,
                rc_version_message.content,
                build_date_message.content,
        ...
        if k == -1:
            val = 1
            await bug_reports_worksheet.update_cell(
                len(reporters) + 1, 1, report_author_verified_name
            )
            ...
            if senior in check_user_roles or frostburn in check_user_roles:
                ...
                for x in config.LIST_OF_LISTS:
                    if x[32] == senior_tester_id:
                        ...
                        break
                self.senior_tester_name = "{0} ({1.name}#{1.discriminator})".format(
                    senior_tester_verified_name, user
                )
        ...
        if reaction.emoji == "❌":
            ...
            new_content = old_content[
                :-41
            ] + "❌ This report has been rejected by {0}.".format(
                self.senior_tester_name
            )
            ...
            new_content = old_content[
                :-41
            ] + "⚠ This report was deemed valid by {0}, but a smilar or related one had already been submitted.".format(
                self.senior_tester_name
            )
            await is_this_valid.edit(content=new_content)
            ...
            new_content = old_content[
                :-41
            ] + "✅ This report has been approved by {0}.".format(
                self.senior_tester_name
            )
            await is_this_valid.edit(content=new_content)
