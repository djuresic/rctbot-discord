...
class Spreadsheet(commands.Cog):
    ...
    async def fetch(self):
        ...
        # Update dynamic
        rctbot.config.LIST_OF_LISTS = await rewards_worksheet.get_all_values()
        rctbot.config.LIST_OF_LISTS_TRIVIA = await trivia_worksheet.get_all_values()
        rctbot.config.SETTINGS = await settings_worksheet.col_values(2)
        rctbot.config.PLAYER_SLASH_HERO = await games_worksheet.col_values(13)

        rctbot.config.DATABASE_READY = True
        rctbot.config.LAST_RETRIEVED = datetime.now(timezone.utc)
