...
class ACPClient:
    ...
    ACP_CONFIG = {
        "ac": {"base_url": f"https://{config.HON_ACP_AC_DOMAIN}", "color": 0x3CC03C},
        "rc": {"base_url": f"http://{config.HON_ACP_RC_DOMAIN}", "color": 0xFF6600},
        "tc": {"base_url": f"http://{config.HON_ACP_TC_DOMAIN}", "color": 0x0059FF},
    }
    ...
    async def close(self):
        await self.request(config.HON_ACP_DEAUTH, method="GET", ssl=self.ssl)
    ...
    async def find_user_path(self, search_by, search_for):
        return await self._search(config.HON_ACP_PROFILE_SEARCH, search_by, search_for)

    async def find_clan_path(self, search_by, search_for):
        return await self._search(config.HON_ACP_CLAN_SEARCH, search_by, search_for)
